...
const GameScene = new Phaser.Class({
  ...
  create() {
    ...
    const controlLine = this.control.create(-100, 100, 'vertical-line').setScale(0.16).setOrigin(0, 0);
    ...
    this.sarsCreate1 = function () {
      if (gameState.control.score > 200) {
        const random = Math.random() - Math.random();
        const virus = gameState.virus1.create(gameState.gameWidth + 200, 320, 'covid-blue').setScale(0.8);
    ...
    function sarsCreate2() {
      if (gameState.control.score > 400) {
        const random = Math.random() - Math.random();
        const virus = gameState.virus2.create(gameState.gameWidth + 200, 320, 'covid-yellow').setScale(0.8);
        virus.setVelocity(-15, 0);
        virus.maxLife = 400;
        virus.life = 400;
        virus.setBounce(1, 1);
    ...
    function sarsCreate3() {
      if (gameState.control.score > 400) {
        const random = Math.random() - Math.random();
        const virus = gameState.virus3.create(gameState.gameWidth + 200, 320, 'covid-green').setScale(0.85);
        virus.setVelocity(-15, 0);
        virus.maxLife = 500;
        virus.life = 500;
        virus.setBounce(1, 1);
    ...
    function glucoseCreate() {
      const random = Math.random() - Math.random();
      const glucose = gameState.glucoses.create(gameState.gameWidth, 180 + Math.random() * 280, 'glucose').setScale(0.1);
    ...
    function interferonCreate() {
      const random = Math.random() - Math.random();
      const interferon = gameState.interferonGammas.create(gameState.gameWidth, 180 + Math.random() * 280, 'interferon').setScale(0.03);
